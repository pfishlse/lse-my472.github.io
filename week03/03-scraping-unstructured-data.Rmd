...
for (i in 2:length(pages)){
  ...
  message(i, '/', length(pages)) #message comes into the console - it's for us
  # prepare URL
  url <- paste0(base_url, pages[i]) #concatenates the url with the number of pages (if that isn't at the end you can add it to the middle/wherever the page number is)
  # scrape website
  data_list[[i]] <- scrape_bribe(url) #adding it to your list. better to do it as items in a list rather than one big table as if your internet cuts out it will save to where you have got to.
